...
if (run ==1){
  ...
  for (i in 1:numIt){
    ...
    #sample generation
    sample <- graph_select(OTUnum = OTUnum, Simulations = Simulations, Strength = Strength, type = "scale-free");
    ...
    if(glasso_indicator){
      ...
      temp <- proc.time()-ptm;
      run_gl <- run_gl + as.double(temp[3]);
    ...
    if(ccrepe_indicator){
      ...
      temp <- proc.time()-ptm;
      run_re <- run_re + as.double(temp[3]);
...
if(eva==1){
  if (glasso_p_indicator){
    ...
    print(sprintf("%.3f (%.3f) & %.3f (%.3f) & %.3f (%.3f)", mean(RMSEgp), sd(RMSEgp), mean(acc), sd(acc), aucgp, aucstdgp))
    ...
    save(perfgp, file = "/Users/chiehlo/Desktop/HMP_project/my_R/performance/synthetic/prior_scale_free_1.RData")
